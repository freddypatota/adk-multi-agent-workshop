...
deploy:
	...
	REGION="us-central1" && \
	gcloud beta run deploy loan-drawdown-agent-demo \
		--source . \
		--memory "4Gi" \
		--project $$PROJECT_ID \
		--region $$REGION \
		...
		--update-env-vars \
		"GOOGLE_GENAI_USE_VERTEXAI=TRUE,LOCATION=us-central1,MODEL_NAME=gemini-2.5-flash,LOGS_BUCKET_NAME=bcr-loan-agent-logs,OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=true" \
		$(if $(IAP),--iap) \
		$(if $(PORT),--port=$(PORT))
