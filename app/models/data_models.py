...
class InvoiceData(BaseModel):
    ...
    is_signed: bool = Field(False, description="Whether the invoice appears to be signed")

class ComplianceCheckResult(BaseModel):
    check_name: str = Field(..., description="Name of the check (e.g., 'Sanctions', 'Prohibited Goods')")
    status: str = Field(..., description="'PASS', 'FAIL'")
    flags: List[str] = Field(default_factory=list, description="List of specific flags or findings")
